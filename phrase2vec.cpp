...
int AddLabel(const char *label) {
  ...
  strcpy_s(labels[label_size], length, label);
  label_size++;
...
int AddWordToSentence(int index, char *word) {
  ...
  strcpy_s(phrases[index].words[sen_index], length, word);
...
int AddSentenceToPhrases(const char *file_name, const char *label) {
  ...
  strcpy_s(phrases[phrase_size].file_name, length, file_name);
  ...
  while (1) {
    ...
    if (entry->attrib & _A_SUBDIR) {
      if (strcmp(label_name, "..") != 0 && strcmp(label_name, ".") != 0) {
        char path[MAX_PATH];
        _snprintf_s(path, MAX_PATH, "%s%s", train_dir, label_name);
        FILE *fin;
        ...
        while (1) {
          ...
          char file[MAX_PATH];
          _snprintf_s(file, MAX_PATH, "%s/%s", path, f->d_name);
...
void LearnVocabFromTestDir() {
  ...
  while (1) {
    ...
    if (entry->attrib & _A_SUBDIR) {
      if (strcmp(label_name, "..") != 0 && strcmp(label_name, ".") != 0) {
        char path[MAX_PATH];
        _snprintf_s(path, MAX_PATH, "%s%s", test_dir, label_name);
        FILE *fin;
...
void *TrainModelThread(void *id) {
  ...
  long long first_phrase = phrase_size / (long long)num_threads * next_random;
  long long last_phrase = phrase_size / (long long)num_threads * (next_random + 1);
  if (debug_mode > 1)
...
void TrainModel() {
  ...
  if (word_output_file[0] != 0) {
    ...
    fopen_s(&fo, word_output_file, "wb");
  ...
  if (phrase_output_file[0] != 0) {
    ...
    fopen_s(&fo, phrase_output_file, "wb");
  ...
  start = clock();
  for (a = 0; a < num_threads; a++) threads2.create_thread(boost::bind(TrainModelThread, (void *)a));//pthread_create(&pt[a], NULL, TrainModelThread, (void *)a);
  threads2.join_all(); //for (a = 0; a < num_threads; a++) pthread_join(pt[a], NULL);
  printf("\nCompleted training. Now outputting vectors.\n");
  fopen_s(&fo, para_file_test, "wb");
  fprintf(fo, "%lld %lld %lld\n", labeled_instances, layer1_size, label_size);
  for (a = 0; a < phrase_size; a++) {
    int* arr = new int[label_size];
    memset(arr, 0, sizeof(arr));
...
int _tmain(int argc, _TCHAR* argv[])
{
  ...
  if ((i = ArgPos((char *)"-train-file", argc, argv)) > 0) strcpy_s(train_file, MAX_STRING, argv[i + 1]);
  if ((i = ArgPos((char *)"-train-dir", argc, argv)) > 0) strcpy_s(train_dir, MAX_STRING, argv[i + 1]);
  if ((i = ArgPos((char *)"-test-file", argc, argv)) > 0) strcpy_s(test_file, MAX_STRING, argv[i + 1]);
  if ((i = ArgPos((char *)"-test-dir", argc, argv)) > 0) strcpy_s(test_dir, MAX_STRING, argv[i + 1]);
  if ((i = ArgPos((char *)"-nn-train", argc, argv)) > 0) strcpy_s(para_file, MAX_STRING, argv[i + 1]);
  if ((i = ArgPos((char *)"-nn-test", argc, argv)) > 0) strcpy_s(para_file_test, MAX_STRING, argv[i + 1]);
  ...
  if ((i = ArgPos((char *)"-output-words", argc, argv)) > 0) strcpy_s(word_output_file, MAX_STRING, argv[i + 1]);
  if ((i = ArgPos((char *)"-output-phrases", argc, argv)) > 0) strcpy_s(phrase_output_file, MAX_STRING, argv[i + 1]);
